...
每个网页打开的时候生成属于自己的clientId，并向后端发起竞争“主”页面请求（也就是让后端保存当前用户的主页面clientId），该请求返回主页面的clientId，各个页面保存主页面clientId，由主页面通过BroadcastChannel向各个页面发送心跳通知（告诉其他从页面，主页面依然存活，当然也要和后端进行心跳通知）如果超时了则从页面可以向后端发起主页面竞争
