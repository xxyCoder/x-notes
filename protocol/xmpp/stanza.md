> 来源：RFC6121 + RFC6120（原RFC3921已被取代），区分「XML语法能力」与「XMPP协议语义约束」
> 语法合法 ≠ 业务可用。
> 通用公理：
> 1. stanza 是 XML 流的一级子元素（流的根是 `<stream:stream>`，stanza 位于 depth=1，不是 XML 文档根元素，RFC6120 §4.1）。
> 2. 属性只能写在开始标签。
> 3. 节点内容分为：文本内容、子元素节点。
> 4. 自定义扩展必须使用**区别于内容命名空间**的独立命名空间（声明形式不限：显式 xmlns、前缀或继承，RFC6120 §8.4）。
> 5. 服务端行为（随角色不同，RFC6120 §8.4 Extended Content）：
>    - 服务器作为**路由者**（目标是其他实体：远程域，或本域账号的在线 full-JID）：未知命名空间的扩展元素原样转发/投递，不解析内部，交给最终接收方处理。
>    - 服务器作为**目标/处理方**（to 为服务器域名、无 to 的服务器直处理、或发往本地 bare-JID 的 iq）：按对应接收规则处理——其中 iq get/set 的未知扩展 **MUST 返回 `service-unavailable`**（RFC6120 §8.4/§10.3.3）。
>    - 不带 xmlns 的子元素：继承**最近祖先作用域中的默认 namespace**（XML Namespaces 默认规则——stanza 直接子层级才是 jabber:client；嵌套在 `<x xmlns="urn:test">` 内的 `<foo/>` 属于 urn:test）——stanza 直接子级的未声明元素**不属于 §8.4 定义的 extension element**（后者必须有区别于内容命名空间的命名空间声明），其处理不能按“扩展透传/忽略”推导；且**自定义**子元素无法被扩展协议识别（`body`/`show` 等标准元素本就通过继承获得内容命名空间，不在此列），**工程上禁止使用**。
...
> 支持0个body，只传递纯结构化扩展数据。

### 完整层级模板

> 模板中的 `<!-- -->` 为**教学注释**：RFC6120 §11.1 禁止在 XML 流中发送注释（接收方应回 `restricted-xml` 流错误），实际报文必须移除。下同。
...
| 项目 | message | presence | iq |
| --- | --- | --- | --- |
| 标准叶子节点 | body、subject、thread（仅文本） | show、status、priority（show/priority 至多一个） | 无标准叶子节点 |
| 扩展在什么位置 | 任意层级均可（§8.4；直接子节点与叶子平级是工程约定） | 同左 | iq下唯一业务载荷（get/set恰好1；result 0~1；error另有`<error/>`） |
| 根下允许多少业务扩展 | 允许多个平级扩展 | 允许多个平级扩展 | 最多1个业务载荷 |
| 扩展内部 | 扩展协议自定义（受 XML 限制） | 扩展协议自定义（受 XML 限制） | 扩展协议自定义（受 XML 限制） |
| 是否强制应答 | 否 | 通常否；probe 例外，按 §4.3.2 分支处理 | get/set 必须应答 |

> 
> 统一铁律：
> 标准叶子标签(body/show/status)只装纯文本；**结构化数据一律放到扩展节点，不要塞进叶子内部**。
> 扩展必须命名空间限定（专属 `xmlns`，通常在根标签显式声明）；扩展内部结构由扩展协议自定义，但仍受 RFC6120 §11.1 的 XML 限制（well-formed、禁 DTD/注释/PI/内部与外部实体引用——预定义实体除外）。
